...
export class InProcessGateway implements Gateway {
  ...
  async *submitTurn(input: GatewaySubmitTurnInput): AsyncIterable<GatewayEvent> {
    ...
    const queue = new AsyncQueue<GatewayEvent>({
      ...
      sizeOf: estimateGatewayEventBytes,
      onOverflow: () => {
        void this.router.abort(input.sessionKey, `gateway_queue_overflow:${runId}`);
      },
    });
      ...
      try {
        ...
        for await (const event of session.submit(
          agentInput,
          {
            ...
            toolCallLimits: input.toolCallLimits,
            permissionMode,
            basePermissionMode,
            allowPlanModeTools,
            permissionRules: {
...
function estimateGatewayEventBytes(event: GatewayEvent): number {
  try {
...
function limitGatewayToolResultPreview(text: string): string {
  return headTailString(text, MAX_GATEWAY_TOOL_RESULT_PREVIEW_CHARS, "Gateway preview truncated");
}
...
function sanitizeGatewayToolDataValue(value: unknown): unknown {
  ...
  if (isRecord(value)) {
    return Object.fromEntries(
      Object.entries(value).map(([key, item]) => [key, sanitizeGatewayToolDataValue(item)]),
    );
  }
  return value;
}
